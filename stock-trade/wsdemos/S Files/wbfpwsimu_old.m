...
if fpwcheckilpass==1
  ...
  if (strcmp(WhereOrderFrom,'SLFE'))
    ...
    if strcmp(fpwrunindex,'11')
      ...
      global intra ycp brkl brks bkei trendv fdctime wstime sj ldcday ltpd wspt cgno
    ...
    if length(stock)>0       
      ...
      if fpwpttype=='P'
        eval(['save ',fpwserverplace,fpwclientdirectory,fpwusername,'\pattern\wbfpwfrontbs frontfpws frontfpwb']);
      end
      ...
      if strcmp(fpwrunindex,'11')
        ...
        if wsmultim==1
          ...
          if (statonly~=1)&(tradeh(1,1)~=0)
            ...
            set(fpwout1.fpwoutfig,'PaperPosition',[.25 .25 4.4 4.25]);
            cd([fpwserverplace,fpwclientdirectory,fpwusername,'\pattern']);
            drawnow;
      ...
      if ~strcmp(fpwrunindex,'11')
        ...
        sfpw.hdfpwptfilefb=frontfpwb; %hdfpwptfilefb;  
        sfpw.hdfpwptfilefs=frontfpws; %hdfpwptfilefs;  
        cd([fpwserverplace,fpwclientdirectory,fpwusername,'\pattern\']);
...
function y=fpwenteb1(stock,i,enp,enw)
   ...
   global sl5 riskd commission tickstep stk dam trendv fdctime wstime sj lotshis MiMi MiMi2 MiMi3 MiMi4 ltdss lcps ltpd tradeh stl sts obl obs tot totc wspt
   global linecodemp; linecodemp=linecodemp(find(linecodemp>=32));
   global wsgp4; wsgp4=stock; wstd350g;
   eval(linecodemp);
