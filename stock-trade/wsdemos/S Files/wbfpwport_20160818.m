  ...
  elseif strcmp(outrunindex,'413')
    ...
    if 0 % this is the old way, only for max model/lots limits
     ...
     if PORUNW=='L'
      ...
      Hhi=0; Iii=0; Jji=0; Kki=0; Lli=0; Mmi=0; Nni=0;
      wbststopritm=0; 
      mwi=ones(1,207);
      ...
      if (pfi==2)|(pfi==4)|(pfi>=6)|(mxp<222)|(mxl<222)|(mxo<222)
        ...
        tpd=0*tradeh(:,1)+1; % trade pass index
        PosiBook=[1 tradeh(1,[13 1]) 0 0 tradeh(1,3)+tradeh(1,9)/24 tradeh(1,2) tradeh(1,5)+tradeh(1,14)/24 1 1 tradeh(1,2)];
        ...
        for j=2:length(TimeA) 
                  ...
                  if (abs(PosiBook(ki,11))>DropNet1)|(DropNet1==0)
                    if (sign(PosiBook(ki,7))==sign(odi))
                      if abs(odi)-odid-abs(PosiBook(ki,7))>=mxp % full shortened!
                        ...
                        NewNet=tradeh(cth,6)-tradeh(cth,2)*(tradeh(cth,4)-LTPrc);
                        ...
                        if fix(TimeA(j))<=tradeh(cth,5)-1
                          lotshis(fix(TimeA(j)):tradeh(cth,5)-1,tradeh(cth,12))=NewSizeNowr*lotshis(fix(TimeA(j)):tradeh(cth,5)-1,tradeh(cth,12));
                        end
                        if fix(TimeA(j))==tradeh(cth,3)
                          stph=1;
                        else
                        ...
                        NewNet=sum(tradeeach(tradeeachi(3):tradeeachi(3)+stph-1))+...
                          ...
                          sign(tradeh(cth,2))*(abs(PosiBook(ki,7))-NewSizeNow)*(tradeh(cth,4)-LTPrc);
                        net1(tradeh(cth,3))=net1(tradeh(cth,3))-tradeh(cth,6)+NewNet; %(count at signal day only)             
                        tradeh(cth,6)=NewNet;
                        net(tradeeach(tradeeachi(1):tradeeachi(2)))=net(tradeeach(tradeeachi(1):tradeeachi(2)))-...
                          tradeeach(tradeeachi(3):tradeeachi(4)); % first drop all
                        tradeeach(tradeeachi(3)+stph-1)=tradeeach(tradeeachi(3)+stph-1)-...
                          (abs(PosiBook(ki,7))-NewSizeNow)*zhusc75(tradeh(cth,13))-...
                          sign(tradeh(cth,2))*(abs(PosiBook(ki,7))-NewSizeNow)*(LTCls-LTPrc);
                        ...
                        if 1 % using the new lot-wised structure
                          if sign(tradeh(timeini(Ii),2))*sign(sum(aPosiBook(:,7)))==1
                            ...
                            if NewSizeNow>0.05 % means more than 5% can be left
                              ...
                              % drop part position
                              lotshis(tradeh(timeini(Ii),3):tradeh(timeini(Ii),5)-1,tradeh(timeini(Ii),12))=NewSizeNowr*lotshis(tradeh(timeini(Ii),3):tradeh(timeini(Ii),5)-1,tradeh(timeini(Ii),12));
          ...
          % checking overnight limits - lots-wised, complicated.
                  ...
                  if (abs(PosiBook(ki,11))>DropNet1)|(DropNet1==0)
                    if (sign(PosiBook(ki,7))==sign(odi))&(fix(PosiBook(ki,8))>fix(TimeA(j)))
                      ...
                      if tradeh(cth,3)==tradeeach(tradeh(cth,10))  % not in at close
                        if abs(odi)-odid-abs(PosiBook(ki,7))>=mxo % full shortened!
                          PosiBook(ki,1)=-1;
                          Ffi=Ffi+1;
                          odid=odid+abs(PosiBook(ki,7));
                          lotshis(fix(TimeA(j)):tradeh(cth,5)-1,tradeh(cth,12))=0*lotshis(fix(TimeA(j)):tradeh(cth,5)-1,tradeh(cth,12));
                          if fix(TimeA(j))==tradeh(cth,3) % meaningless at here
                            lotshis(fix(TimeA(j)),tradeh(cth,12))=tradeh(cth,2);
                            stph=1;
                          else
                            lotshis(tradeh(cth,3):fix(TimeA(j))-1,tradeh(cth,12))=0*lotshis(tradeh(cth,3):fix(TimeA(j))-1,tradeh(cth,12))+tradeh(cth,2);
                        ...
                        if abs(odi)-odid-abs(PosiBook(ki,7))>=mxo % so simply as no trade
                          PosiBook(ki,1)=-1;
                          tpd(cth)=0; Iii=Iii+1;
                          odid=odid+abs(PosiBook(ki,7));
      ...
      if 0 % (pfi==1)|(pfi==7)|(pfi==4)|(pfi==5)|(mxv<222) % old model-wise atructure
      ...
      elseif (mxv<222) % lots-wise structure
    ...
    if 0 % for old ways
      ...
      s.StatusReport=[PORUNW,'-Based Run: GF:',int2str(Jji),', Overlap:',int2str(Cci),...
      ', MxP:',int2str(Hhi),'/',int2str(Ddi),', MxL:',int2str(Ggi),', MxONight:',int2str(Ffi),'/',int2str(Kki),...
      ', ',int2str(Iii),'/',int2str(Eei),', VCAed:',int2str(Bbi),', ONAdd:',int2str(Lli),...
      ', ExitOut:',int2str(Mmi),'/',int2str(Nni)];
      if DropNet1>0
