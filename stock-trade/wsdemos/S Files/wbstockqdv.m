function retstr = wbstockqdv(instruct, outfile)

% Author(s): N. Zhu, 11-15-2003
% This is a WB (Web-Based) mirror program to <stockqdv.m> in desktop version
...
if (length(stockday)>0)|(length(testdata)>0)
  ...
  Fig3=figure('visible','off','posi',[2 3 600 180],'inverthardcopy','off','color','k');
  zhu27r=axes('position',[0.091 0.353 .89 .59]);
  zhu28r=axes('position',[0.091 0.091 .89 .22]);
  ...
  subplot(zhu28r);
  %axis([0 yrs+1 0 1.1*max(vol)])
  barg(vol(2:yrs+1))
  hold on
  %xlabel(['Last: ',price,',  Net: ',net]);
  grid;
  ylabel('Vol in K')
  set(get(gca,'xlabel'),'fontsize',11);set(get(gca,'ylabel'),'fontsize',11);
  chgst93(datem(2:length(datem(:,1)),:),1); hold off
  ...
  set(gcf,'PaperPosition',[.25 .25 8.05 4.5]);
  cd([fpwserverplace,fpwclientdirectory,fpwusername,'\stock']);
  Plotfileh=[fpwclientdirectory,fpwusername,'\stock\wbstockqdv3.jpeg'];
  drawnow;
  wsprintjpeg(Fig3,'wbstockqdv3.jpeg');
  close(Fig3);
  ...
  Fig2=figure('visible','off','posi',[2 3 600 300],'inverthardcopy','off','color','k');
  zhu27rr=axes('position',[0.091 0.353 .89 .59]);
  zhu28rr=axes('position',[0.091 0.091 .89 .22]);
  ...
  barg(vol(2:yrs+1))
  hold on
  avg=fix(mean(vol(1:yrs-1))*100)/100;
  avgs=num2str(avg*100);
  lasts=num2str(fix(vol(yrs+1)*100));
  changes=int2str(fix(100*vol(yrs+1)/avg-100));
  plot(1:yrs+1,avg*ones(1,yrs+1),'-r')
  %xlabel(['Average: ',avgs,',  Today: ',lasts,',  Change: ',changes,'%']);
  xlabelstr=['Average: ',avgs,'K,  Today: ',lasts,'K,  Change: ',changes,'%'];
  ylabel('Vol in 100K')
  ...
  set(gca,'Xcolor',[0.5 0.5 0.5]);
  set(gca,'Ycolor',[0.5 0.5 0.5]);   
  ftzn=figtext(0.89,0.96,date2str(timevaluea));
  set(ftzn,'color',[0.65 0.65 0.65],'fontsize',8);
  ftzn=figtext(0.02,0.03,['Copyright ',datestr(now,10),' WSQSI, http://www.WSQSI.com/']);
  set(ftzn,'color',[0.5 0.5 0.5],'fontsize',7);
  ftzn1=figtext(0.35,0.03,xlabelstr);
  set(ftzn1,'color',[0.5 0.5 0.5],'fontsize',10);  
  set(gcf,'PaperPosition',[.25 .25 8.05 4.50]);
  cd([fpwserverplace,fpwclientdirectory,fpwusername,'\stock']);
  Plotfileh=[fpwclientdirectory,fpwusername,'\stock\wbstockqdv2.jpeg'];
  drawnow;
  wsprintjpeg(Fig2,'wbstockqdv2.jpeg');
  close(Fig2);
  ...
  Fig1=figure('visible','off','posi',[2 3 600 180],'inverthardcopy','off','color','k');
  zhu27R=axes('position',[0.091 0.353 .89 .59]);
  zhu28R=axes('position',[0.091 0.091 .89 .22]);  
  datem=stockday(:,1:3);
  stock=stockday(:,4:8);
  ...
  subplot(zhu28R);
  dys=length(vol);
  axis([0 dys 0 1.1*max(vol)])
  avg=fix(mean(vol(max([1 dys-250]):dys-2))*100)/100;
  avgs=num2str(avg*100);
  lasts=num2str(fix(vol(dys-1)*100));
  changes=int2str(fix(100*vol(dys-1)/avg-100));
  plot(1:dys,avg*ones(size(1:dys)),'-r');hold on;
  X1=1:dys;
  X(1:dys,1:2)=[X1' X1'];
  line(1:dys,1:2)=[vol*0 vol];
  for I=1:dys
    plot(X(I,:),line(I,:),'-g');
  end
  %xlabel(['52 Wks Average: ',avgs,', Yesterday: ',lasts,', Chge: ',changes,'%']);
  xlabelstr=['52 Wks Average: ',avgs,'K, Yesterday: ',lasts,'K, Chge: ',changes,'%'];
  ylabel('Vol in 100K')
  %set(get(gca,'xlabel'),'fontsize',11);
  set(get(gca,'ylabel'),'fontsize',11);
  chgst93(datem);
  ...
  set(gcf,'PaperPosition',[.25 .25 8.05 4.5]); 
  cd([fpwserverplace,fpwclientdirectory,fpwusername,'\stock']);
  Plotfileh=[fpwclientdirectory,fpwusername,'\stock\wbstockqdv1.jpeg'];
  drawnow;
  wsprintjpeg(Fig1,'wbstockqdv1.jpeg');
  close(Fig1);
...
templatefile = which('wbstockqdv.html');
