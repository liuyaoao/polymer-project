function retstr = wbdzhjust(instruct, outfile)
...
fseek(fid1,-50,1); fprintf(fid1,[time,' MPGF\n']); fclose(fid1);
clear fid1
...
global PE CAP SInd KWIN RmSI RmKW %SInd - Sector Index, KWIN - Key Words In company Names, RmSI - ReMoving Sector Index
...
if (fpwscpass1+fpwscpass2+fpwscpass3)~=3
  ...
  if (length(ip)>0)&(strcmp(upper(fpwusername),'NINGZHU')==1)
    fpwsnltbd=ip;
    eval(['save ',Wherematlab,'stock\fpwsnltbd fpwsnltbd']);
  end

  y={};i=0;
  if (length(ip)>0)
    if ohlcvn(ip(1),6)<0
      y{1,1}=sprintf('<font color="blue" size="2" face="Arial"><b>%5s</b></font>',['G',num2str(length(ip))]); 
      ...
      y{1,10}=sprintf('<a href="Javascript: stockRForm.mlmfvar2.value=''5''; stockRForm.mlmfile.value=''wbstmpsort''; stockRForm.submit();"><font color="blue" size="2" face="Arial"><b>%5s</b></font></a>','FILT'); 
    else
      y{1,1}=sprintf('<font color="red" size="2" face="Arial"><b>%5s</b></font>',['G',num2str(length(ip))]); 
      ...
      y{1,10}=sprintf('<a href="Javascript: stockRForm.mlmfvar2.value=''5''; stockRForm.mlmfile.value=''wbstmpsort''; stockRForm.submit();"><font color="red" size="2" face="Arial"><b>%5s</b></font></a>','FILT');   
    end
  else
    ...
    y{1,10}=sprintf('<font color="red" size="2" face="Arial"><b>%5s</b></font>','FILT');       
  end
  ...
  y{1,14}=[sprintf('<font color="blue" size="2" face="Arial"><b>%5s</b></font>',['[GF-',time1,']',blanks(20)]),...
...
wbfilter='NA'; wbscankind='FILT';
PeCaSe=[PE(ip) CAP(ip) sectorind(ip)]; y11cell='G';
