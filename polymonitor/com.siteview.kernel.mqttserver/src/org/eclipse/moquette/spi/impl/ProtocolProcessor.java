...
class ProtocolProcessor implements EventHandler<ValueEvent> {
    ...
    void processConnect(ServerChannel session, ConnectMessage msg) {
        ...
        if(!OnlineStatus.getInstance().onlines.contains(msg.getClientID())){
        	 OnlineStatus.getInstance().onlines.offer(msg.getClientID());
//        	 System.out.println("online client[processConnect] - " +  OnlineStatus.getInstance().onlines.toString());
        }
...
//        	System.out.println(String.format("Can't find a ConnectionDescriptor for client <%s> in cache <%s>", clientId, m_clientIDs));
...
//        m_clientIDs.get(clientID).getSession().write(pubRelMessage);
    ...
    void processDisconnect(ServerChannel session, DisconnectMessage msg) throws InterruptedException {
        ...
        OnlineStatus.getInstance().onlines.remove(clientID);
//        System.out.println("online client[processDisconnect] - " +  OnlineStatus.getInstance().onlines.toString());
...
//        m_notifier.disconnect(evt.getSession());
    ...
    void processConnectionLost(LostConnectionEvent evt) {
        ...
        OnlineStatus.getInstance().onlines.remove(clientID);
//        System.out.println("online client[processConnectionLost] - " +  OnlineStatus.getInstance().onlines.toString());
