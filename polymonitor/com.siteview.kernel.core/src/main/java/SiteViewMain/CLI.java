...
public class CLI extends Thread
{
    ...
    void handleRequest(String s)
    {
        ...
        String port = TextUtils.getValue(hashmap, "-port");
        String server = TextUtils.getValue(hashmap, "-server");
        String username = TextUtils.getValue(hashmap, "-username");
        String password = TextUtils.getValue(hashmap, "-password");
        String proxy = TextUtils.getValue(hashmap, "-proxy");
        String proxyPassword = TextUtils.getValue(hashmap, "-proxyPassword");
        String proxyUsername = TextUtils.getValue(hashmap, "-proxyUsername");
        int timeout = TextUtils.toInt(TextUtils.getValue(hashmap, "-timeout"));
        if(timeout <= 0)
            timeout = 60;
        timeout *= 1000;
        String page = TextUtils.getValue(hashmap, "-page");
        int maxSize = TextUtils.toInt(TextUtils.getValue(hashmap, "-maxSize"));
        if(maxSize <= 0)
            maxSize = 50000;
        String request = "http://" + server + ":" + port + "/SiteView/cgi/go.exe/SiteView";
        ...
        long al[] = URLMonitor.checkURL(socketsession, request, "", "", proxy, proxyUsername, proxyPassword, array, username, password, "", stringbuffer1, maxSize, "", 0, timeout, null);
        long l = al[0];
