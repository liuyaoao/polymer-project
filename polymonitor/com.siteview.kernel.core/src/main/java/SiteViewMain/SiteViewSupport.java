...
//                LogManager.registerLogger("SiteViewLog", jdbclogger);
    ...
    {
        String groupsbackup = Platform.getRoot() + "/groupsbackup";
        String groups = Platform.getRoot() + "/groups";
        cleanDir(groupsbackup);
        File file = new File(groups);
        String as[] = file.list();
        File file1 = new File(groupsbackup);
        if(!file1.exists())
            file1.mkdir();
        for(int i = 0; as != null && i < as.length; i++)
        {
            File file2 = new File(groups, as[i]);
            if(!file2.exists())
                continue;
            try
            {
                StringBuffer stringbuffer = FileUtils.readFile(groups + "/" + as[i]);
                FileUtils.writeFile(groupsbackup + "/" + as[i], stringbuffer.toString());
            }
            catch(Exception exception)
            {
                System.out.println("Could not backup file: " + as[i] + " to: " + groupsbackup + " Error: " + exception.getMessage());
            }
        }

    }

    private static void cleanDir(String groupsbackup)
    {
        File file = new File(groupsbackup);
        if(file.exists())
        {
            String s1 = "attrib -R " + groupsbackup + "/*.*";
            if(!Platform.isWindows())
                s1 = "chmod -R u+rw " + groupsbackup;
...
//            Array array = commandline.exec(s1);
            ...
            for(int i = 0; i < as.length; i++)
            {
                ...
                File file1 = new File(groupsbackup, as[i]);
                if(!file1.exists())
                    continue;
                try
                {
