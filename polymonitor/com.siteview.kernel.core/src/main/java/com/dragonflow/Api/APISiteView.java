...
//    }
    ...
    protected static void initSSChildObjects()
    {
        if(ssChildObjects == null && siteViewObjects == null)
        {
            ...
            try
            {
                java.io.File file = new File(Platform.getRoot() + "/target/classes/com/dragonflow/Api");
                String as[] = file.list();
                for(int i = 0; i < as.length; i++)
                {
    ...
    protected static jgl.Array getObjects(String s)
    {
        ...
        java.io.File file = new File(Platform.getRoot() + "/target/classes/com/dragonflow/Standard" + s);
        String as[] = file.list();
        if(as != null)
        {
            for(int i = 0; i < as.length; i++)
...
//        boolean flag1 = s.indexOf("NT") == -1;
    ...
    private void enableHeartbeats(String s, int i, long l)
    {
        ...
        {
            jgl.HashMap hashmap = MasterConfig.getMasterConfig();
            String s1 = TextUtils.getValue(hashmap, "_sseeHeartbeatTimer");
            if(s1 != null && s1.length() > 0)
            {
