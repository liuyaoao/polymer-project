...
public abstract class LogMonitorBase extends ServerMonitor {
    ...
    long[] checkLog(StringBuffer stringbuffer, long l) {
        ...
        try {
            ...
            if (0L <= startSearchPosition && startSearchPosition < endSearchPosition) {
                ...
                boolean mlRegEx = TextUtils.isMultiLineRegularExpression(getProperty(getMatchProperty()));//http://www.siteview.com
    ...
    String readRules() {
        String s = getProperty(getRulesFileProperty());//_rulesFile
        if (s.length() == 0) {
            return "";
        }
        usingRules = true;
