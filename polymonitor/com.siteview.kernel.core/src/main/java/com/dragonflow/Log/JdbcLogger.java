...
public class JdbcLogger extends com.dragonflow.Log.Logger
{
    ...
    public JdbcLogger(jgl.HashMap hashmap)
    {
        ...
        logger = this;
        logCache = new Array();
        method = TextUtils.getValue(hashmap, "_logJdbcMethod" + logName);
    ...
    {
        String s = Platform.getRoot() + java.io.File.separator + "groups" + java.io.File.separator + "backupdatabase.enabled";
        ...
        throws java.lang.Exception
    {
        if(connection == null)
        {
            if(TextUtils.getValue(hashmap, "_logJdbcDebug").length() > 0)
            {
                debug = true;
            }
            if(TextUtils.getValue(hashmap, "_logJdbcDebugOutput").length() > 0)
            {
                java.sql.DriverManager.setLogStream(System.out);
            }
            String s1 = TextUtils.getValue(hashmap, "_logJdbcURLBackup" + logName);
            usingBackupSignalFile = TextUtils.getValue(hashmap, "_logJdbcBackupSignal").length() > 0;
            ...
            driver = TextUtils.getValue(hashmap, "_logJdbcDriver" + logName);
            String s2 = TextUtils.getValue(hashmap, "_logJdbcUser" + logName);
            String s3 = TextUtils.getValue(hashmap, "_logJdbcPassword" + logName);
            String s4 = TextUtils.getValue(hashmap, "_logJdbcPropertyFilter" + logName);
            if(s4.length() > 0)
            {
                propertyFilter = com.dragonflow.Utils.TextUtils.split(s4, ",");
                com.dragonflow.Log.LogManager.log("RunMonitor", "jdbc log, filter " + s4);
            }
            String s5 = TextUtils.getValue(hashmap, "_logJdbcPropertySkip" + logName);
            if(s5.length() > 0)
            {
                propertySkip = com.dragonflow.Utils.TextUtils.split(s5, ",");
                com.dragonflow.Log.LogManager.log("RunMonitor", "jdbc log, skip " + s5);
            }
            int i = com.dragonflow.Utils.TextUtils.toInt(TextUtils.getValue(hashmap, "_logJdbcMaxString" + logName));
            if(i > 0)
            {
                maxStringLength = i;
            }
            autoCommit = com.dragonflow.Utils.TextUtils.toInt(TextUtils.getValue(hashmap, "_logJdbcAutoCommit" + logName));
            if(autoCommit == 0)
            {
                autoCommit = 20;
            }
            reconnectSeconds = com.dragonflow.Utils.TextUtils.toInt(TextUtils.getValue(hashmap, "_logJdbcReconnect" + logName));
...
//                ((sun.jdbc.odbc.JdbcOdbcPreparedStatement)preparedstatement).FreeParams();
        ...
        if(allowProperty(propertiedobject, stringproperty2))
        {
            ...
            while (enumeration1.hasMoreElements())
                ...
                if(k <= vars)
                {
                    ...
                    if(stringproperty3 == AtomicMonitor.pName)
                    {
        ...
        throws java.lang.Exception
    {
        ...
        if(logStatementCache == null)
        {
            ...
            String s = TextUtils.getValue(hashmap, "_logJdbcURL" + logName);
            if(s.length() == 0)
            {
                return null;
            }
            setupConnection(hashmap, s);
            String s1 = TextUtils.getValue(hashmap, "_logJdbcInsert" + logName);
            String s2 = TextUtils.getValue(hashmap, "_logJdbcCreate" + logName);
            method = TextUtils.getValue(hashmap, "_logJdbcMethod" + logName);
        ...
        throws java.lang.Exception
    {
        if(linkStatementCache == null)
        {
            ...
            setupConnection(hashmap, s);
            String s1 = TextUtils.getValue(hashmap, "_logJdbcInsertLink");
            if(s1.length() == 0)
            {
                s1 = "INSERT INTO SiteViewLinks VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            }
            String s2 = TextUtils.getValue(hashmap, "_logJdbcCreateLink");
            if(s2.length() == 0)
            {
            ...
            linkServer = SiteViewGroup.getServerID(hashmap);
            java.sql.Statement statement = null;
            try
            {
        ...
        throws java.lang.Exception
    {
        ...
        if(obj == null)
        {
            ...
            jgl.HashMap hashmap = MasterConfig.getMasterConfig();
            String s6 = TextUtils.getValue(hashmap, "_logJdbcURL" + logName);
            if(s6.length() == 0)
            {
                return null;
            }
            setupConnection(hashmap, s6);
            String s7 = TextUtils.getValue(hashmap, "_logJdbcInsert" + s);
            if(s7.length() == 0)
            {
                s7 = s5;
            }
            String s8 = TextUtils.getValue(hashmap, "_logJdbcDefine" + s);
            if(s8.length() == 0)
            {
                s8 = s4;
            }
            String s9 = TextUtils.getValue(hashmap, "_logJdbcCreate" + s);
            if(s9.length() == 0)
            {
                s9 = s1;
            }
            String s10 = TextUtils.getValue(hashmap, "_logJdbcCreate2" + s);
            if(s10.length() == 0)
            {
                s10 = s2;
            }
            String s11 = TextUtils.getValue(hashmap, "_logJdbcCreate3" + s);
            if(s11.length() == 0)
            {
                s11 = s3;
            }
