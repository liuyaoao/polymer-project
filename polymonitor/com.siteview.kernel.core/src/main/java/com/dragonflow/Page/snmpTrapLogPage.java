...
public class snmpTrapLogPage extends com.dragonflow.Page.CGI
{
        ...
        throws java.lang.Exception
    {
        ...
        String s = request.getValue("records");
        String s1 = request.getValue("match");
        printBodyHeader("SNMP Trap Log");
        printButtonBar("SNMPTrapLog.htm", "");
        String s2 = com.dragonflow.StandardMonitor.SNMPTrapMonitor.listenerStatus;
        ...
        if(!isPortalServerRequest())
        {
            ...
            if(file.exists())
            {
                ...
                String s5 = braf.readLine();
                if(s5 != null)
                {
                    do
                    {
                        String s6 = braf.readLine();
                        if(s6 == null)
                        {
                            break;
                        }
    ...
    void printTraps(jgl.Array array, int i, java.io.PrintWriter printwriter)
    {
        ...
        do
        {
            ...
            do
            {
                ...
                if(s2.length() > 0)
                {
                    String s4 = TextUtils.getValue(hashmap, s2);
                    if(s4.length() > 0)
                    {
                        s3 = s4 + ", " + s3;
                    }
            ...
            printwriter.println("<tr><td>" + TextUtils.getValue(hashmap, "date") + "<td>" + TextUtils.getValue(hashmap, "from") + "<td>" + TextUtils.getValue(hashmap, "vars") + "<td>" + TextUtils.getValue(hashmap, "trap") + "<td>" + TextUtils.getValue(hashmap, "specific") + "<td>" + TextUtils.getValue(hashmap, "oid") + "<td>" + TextUtils.getValue(hashmap, "agent") + "<td>" + TextUtils.getValue(hashmap, "community") + "<td>" + TextUtils.getValue(hashmap, "traptime"));
        } while(++j < i);
