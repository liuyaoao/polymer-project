...
public abstract class remoteBase extends com.dragonflow.Page.prefsPage {
	...
	void formLogin(jgl.HashMap hashmap) {
		...
		outputStream.println(field("Login",
				"<input type=text name=login size=50 value=\"" + TextUtils.getValue(hashmap, "_login") + "\">", s1));
		String s2 = "";
		if (TextUtils.getValue(hashmap, com.dragonflow.SiteView.Machine.pPassword.getName()) != "") {
			s2 = "*********";
		}
		outputStream.println(field("Password", "<input type=password name=password size=50 value=\"" + s2 + "\">",
				"The password for the remote server or the passphrase for the SSH key file."));
		outputStream.println(field("Title",
				"<input type=text name=title size=50 value=\""
						+ com.dragonflow.Page.remoteBase.getValue(hashmap, "_name") + "\">",
    ...
    String actionsListStr = JSONObject.toJSONString(actionsList);
		...
		outputStream.println("<simple-data-table-ext data-list="+actionsListStr+" is-show-header no-data-tip='no remote servers defined'></simple-data-table-ext><br>");

		if (request.actionAllowed("_preference") || s4.length() > 0) {
			outputStream.println("<A HREF=" + getPageLink(getPage(), "Add") + getReturnLink(false, s4)
					+ ">Add</A> a Remote Machine\n" + "<br><br>\n");
		}
		if (s4.length() > 0) {
			String s2 = new String(
					com.dragonflow.Page.serverPage.getReturnURL(com.dragonflow.Page.serverPage.thisURL, s4));
			outputStream.println("<a href=\"" + s2 + "&pop=true&storeID=" + s4 + "\">Return to Choose Server</a>");
		}
		printFooter(outputStream);
	}
	...
	void printForm(String s, jgl.Array array, jgl.HashMap hashmap) throws java.lang.Exception {
		...
		outputStream.println("<input type=hidden name=backURL value=\"" + request.getValue("backURL") + "\">\n"
		...
		outputStream.println("</TABLE>");
		if (flag) {
			outputStream.println("<TABLE WIDTH=100%><TR><TD>" + getReturnLink(true, s4) + "<input type=submit value=\""
	...
	void printDeleteForm(String s) throws java.lang.Exception {
		...
		if (request.isPost()) {
			...
			outputStream.println("<FONT SIZE=+1>Are you sure you want to delete the remote server <B>" + s2
					...
					+ getPageLink(getPage(), "List") + ">Return to Detail</A></TD><TD WIDTH=6%></TD>"
					+ "</TR></TABLE></FORM>");
			printFooter(outputStream);
		}
	}
	...
	public void printTestForm(String s) {
		...
		outputStream.println("<table border=\"0\" width=\"99%\"><tr><td width=\"50%\">&nbsp;</td><td align=\"right\">");
		printBackLink();
	...
	protected String getSshAuthMethodHtml(jgl.HashMap hashmap) {
		...
		return field("SSH Authentication Method",
				"<select name=sshAuthMethod size=1>" + com.dragonflow.Page.remoteBase
						.getOptionsHTML(com.dragonflow.SiteView.Machine.getAllowedSshAuthMethods(), s),
	...
	protected StringBuffer getAdnvacedSSHOptions(jgl.HashMap hashmap) {
		...
		stringbuffer.append(
				"<TR><TD ALIGN=\"CENTER\" VALIGN=\"CENTER\" colspan=3><HR><H3>SSH Advanced Options</H3></TD></TR>");
