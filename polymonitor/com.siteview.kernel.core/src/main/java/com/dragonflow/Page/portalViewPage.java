...
public class portalViewPage extends com.dragonflow.Page.portalPreferencePage
{
        ...
        throws java.io.IOException
    {
        ...
        for(int i = 0; i < ahashmap.length; i++)
        {
            for(int j = 0; j < ahashmap[i].length; j++)
            {
                ...
                if(hashmap != null)
                {
                    int k = 1 + (com.dragonflow.Utils.TextUtils.toInt(TextUtils.getValue(hashmap, "yEnd")) - com.dragonflow.Utils.TextUtils.toInt(TextUtils.getValue(hashmap, "yStart")));
                    int l = 1 + (com.dragonflow.Utils.TextUtils.toInt(TextUtils.getValue(hashmap, "xEnd")) - com.dragonflow.Utils.TextUtils.toInt(TextUtils.getValue(hashmap, "xStart")));
                    String s1 = "";
                    String s2 = "";
                    if(k > 1)
                    {
    ...
    void printCell(jgl.HashMap hashmap, int i, int j, String s)
        throws java.io.IOException
    {
        String s1 = TextUtils.getValue(hashmap, "view");
        String s2 = TextUtils.getValue(hashmap, "query");
        String s3 = "/\\.(html|htm|xsl|gif|jpg)$/";
        jgl.Array array = Portal.getEditableViewContentsArray(s3, true);
        jgl.Array array1 = Portal.getEditableQueryArray();
        ...
        outputStream.println("<TABLE BORDER=0><INPUT TYPE=HIDDEN NAME=" + s4 + "xStart VALUE=" + TextUtils.getValue(hashmap, "xStart") + ">" + "<INPUT TYPE=HIDDEN NAME=" + s4 + "xEnd VALUE=" + TextUtils.getValue(hashmap, "xEnd") + ">" + "<INPUT TYPE=HIDDEN NAME=" + s4 + "yStart VALUE=" + TextUtils.getValue(hashmap, "yStart") + ">" + "<INPUT TYPE=HIDDEN NAME=" + s4 + "yEnd VALUE=" + TextUtils.getValue(hashmap, "yEnd") + ">" + "<TR><TD>Cell contents:</TD><TD><select name=" + s4 + "view>" + com.dragonflow.Page.portalViewPage.getOptionsHTML(array, s1) + "</select></TD><tr></tr><TD colspan=2><font size=\"-1\">Select a HTML or XSL template (required)</font></TD></TR>" + "<TR><TD>Data query:</TD><TD><select name=" + s4 + "query>" + com.dragonflow.Page.portalViewPage.getOptionsHTML(array1, s2) + "</select></TD><tr></tr><TD colspan=2><font size=\"-1\">Named Query for XML/XSL (optional)</font></TD></TR>" + "<TR><TD>Focus:</TD><TD><input type=text name=" + s4 + "focus " + "value=\"" + TextUtils.getValue(hashmap, "focus") + "\"></TD>" + "<tr></tr><TD colspan=2><font size=\"-1\">Optional data focus variable</font></TD></TR>" + "<TR><TD>Level(s):</TD><TD><select name=" + s4 + "hint>" + com.dragonflow.Page.portalViewPage.getOptionsHTML(array2, TextUtils.getValue(hashmap, "hint")) + "</select></TD><tr></tr><TD colspan=2><font size=\"-1\">Optional XML data trimming </font></TD></TR>" + "</TABLE>");
    }
    ...
    {
        jgl.Array array = Portal.getEditableViewArray();
        jgl.Array array1 = new Array(array);
        array1.pushFront("Top-Level");
        array1.pushFront("");
        return "<select size=\"1\" name=\"parentViewID\"\n>" + com.dragonflow.Page.portalViewPage.getOptionsHTML(array1, TextUtils.getValue(hashmap, "_parentID")) + "</SELECT>";
    }
        ...
        throws java.io.IOException
    {
        ...
        {
            outputStream.println("<TABLE>");
            jgl.Array array = Portal.getEditableViewContentsArray("/^Frame.*htm[l]*$/", true);
        ...
        } else
        {
            ...
            jgl.Array array1 = Portal.getEditableViewContentsArray("Frame", true);
            if(TextUtils.getValue(hashmap, "_framesHTML").length() > 0)
            {
    ...
    {
        outputStream.println("<TABLE>");
        jgl.Array array = Portal.getEditableViewContentsArray("Header", true);
    ...
    jgl.HashMap fillInResultFrame(jgl.HashMap hashmap)
    {
        ...
        for(Enumeration enumeration = hashmap.values("_cell"); enumeration.hasMoreElements(); hashmap1.put(s2, hashmap2))
        {
            ...
            s2 = TextUtils.getValue(hashmap2, "xStart") + "," + TextUtils.getValue(hashmap2, "yStart") + "," + TextUtils.getValue(hashmap2, "xEnd") + "," + TextUtils.getValue(hashmap2, "yEnd");
        }
    ...
    void printListItem(jgl.HashMap hashmap)
    {
        ...
        if(!s1.equals(""))
        {
            if(TextUtils.getValue(hashmap, "_package").length() > 0)
            {
                s5 = "&nbsp;";
                s6 = "&nbsp;";
                s1 = s1 + " (in <B>" + TextUtils.getValue(hashmap, "_package") + "</B> package)";
            }
