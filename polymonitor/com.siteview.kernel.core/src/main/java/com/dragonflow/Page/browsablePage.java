...
public class browsablePage extends com.dragonflow.Page.CGI
{
        ...
        throws java.lang.Exception
    {
        ...
        if(s1.length() == 0)
        {
            ...
            atomicmonitor = AtomicMonitor.MonitorCreate(array, s3, request.getPortalServer(), request);
            s1 = (String)atomicmonitor.getClassProperty("class");
        } else
        {
            atomicmonitor = AtomicMonitor.MonitorCreate(s1, request);
        }
        if(atomicmonitor instanceof com.dragonflow.SiteView.BrowsableMonitor)
        {
            ...
            if((atomicmonitor instanceof com.dragonflow.SiteView.IServerPropMonitor) && request.hasValue("server"))
            {
                ...
                jgl.HashMap hashmap3 = BrowsableCache.getCache(request.getValue("uniqueID"), false, false);
            ...
            if(flag)
            {
                ...
                if(operation.equals("Choose"))
                {
                    ...
                    jgl.Array array1 = BrowsableCache.getSelections(request.getValue("uniqueID"), true, true);
                    jgl.HashMap hashmap6 = (jgl.HashMap)array1.at(0);
                    com.dragonflow.SiteView.BrowsableCache.mergeSelections((com.dragonflow.SiteView.BrowsableMonitor)atomicmonitor1, request.getValue("uniqueID"), false, "visible", request, false);
                    jgl.Array array2 = BrowsableCache.getSelections(request.getValue("uniqueID"), true, false);
				...
				if(operation.equals("Browse"))
                {
					try
					{					
	                    jgl.HashMap hashmap4 = BrowsableCache.getCache(request.getValue("uniqueID"), false, false);
                ...
                } else
                {
                    ...
                    {
                        jgl.HashMap hashmap8 = BrowsableCache.getCache(request.getValue("uniqueID"), false, false);
                        ...
                        if(flag1)
                        {
                            s12 = BrowsableCache.getXmlFileName(atomicmonitor1);
                            ...
                            {
                                s10 = BrowsableCache.getXmlFile(s12);
                            }
                        }
                        if(s10.length() == 0)
                        {
                    ...
                    {
                        document = BrowsableCache.getXml(request.getValue("uniqueID"));
                    }
                    if(stringbuffer.length() > 0)
                    {
    ...
    void printConnectionProperties(com.dragonflow.SiteView.AtomicMonitor atomicmonitor, jgl.HashMap hashmap, StringBuffer stringbuffer)
    {
        ...
        jgl.HashMap hashmap1 = BrowsableCache.getCache(request.getValue("uniqueID"), false, false);
        ...
        throws java.lang.Exception
    {
        if(document.getDocumentElement() != null)
        {
            try
            {
                String s = "";
                boolean flag = false;
                jgl.HashMap hashmap = BrowsableCache.getCurrentState(request.getValue("uniqueID"));
                ...
                do
                {
                    ...
                    if(s1.startsWith(OPEN))
                    {
                        ...
                        s = s2;
                        if(!TextUtils.getValue(hashmap, s2).equals(OPEN))
                        {
                            hashmap.put(s2, OPEN);
                            flag = true;
                        }
                    ...
                    if(s1.startsWith(CLOSE))
                    {
                        ...
                        s = s3;
                        if(!TextUtils.getValue(hashmap, s3).equals(CLOSE))
                        {
                            hashmap.remove(s3);
                            flag = true;
                        }
                ...
                jgl.Array array = BrowsableCache.getSelections(request.getValue("uniqueID"), true, false);
                org.w3c.dom.NodeList nodelist = document.getDocumentElement().getChildNodes();
                int i = nodelist.getLength();
                long l = 0L;
                if(debug)
