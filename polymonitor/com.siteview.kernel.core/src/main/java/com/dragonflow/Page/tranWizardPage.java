...
public class tranWizardPage extends com.dragonflow.Page.monitorPage
{
    ...
    void printVars(String s)
    {
        ...
        String s1;
        for(Enumeration enumeration = request.getVariables(); enumeration.hasMoreElements(); System.out.println(s1 + "=" + request.getValue(s1)))
        {
    ...
    Enumeration getForms(com.dragonflow.Utils.HTMLTagParser htmltagparser, com.dragonflow.SiteView.AtomicMonitor atomicmonitor)
    {
        ...
        do
        {
            ...
            i++;
            jgl.HashMap hashmap = (jgl.HashMap)enumeration.nextElement();
            String s = TextUtils.getValue(hashmap, "name").trim();
            if(s.length() == 0)
            {
                s = TextUtils.getValue(hashmap, "action").trim();
            }
            s = "[" + i + "]" + s;
            ...
            do
            {
                ...
                String s7 = "";
                jgl.HashMap hashmap1 = (jgl.HashMap)enumeration1.nextElement();
                String s8 = TextUtils.getValue(hashmap1, "tag");
                if(s8.equals("select"))
                {
                    String s9 = null;
                    String s12 = null;
                    ...
                    do
                    {
                        ...
                        {
                            s9 = TextUtils.getValue(hashmap2, "value");
                        }
                    } while(true);
                    if(s9 == null)
                    {
                    ...
                    s5 = TextUtils.getValue(hashmap1, "name");
                    s7 = s9;
                } else
                {
                    String s10 = TextUtils.getValue(hashmap1, "type").toLowerCase();
                    ...
                    {
                        j++;
                        s2 = TextUtils.getValue(hashmap1, "name");
                        if(s2.length() == 0)
                        {
                            s2 = TextUtils.getValue(hashmap1, "alt");
                            if(s2.length() == 0)
                            {
                                s2 = TextUtils.getValue(hashmap1, "value");
                                if(s2.length() == 0)
                                {
                                    s2 = TextUtils.getValue(hashmap1, "src");
                                    if(s2.length() == 0)
                                    {
                                        s2 = "[" + j + "]";
                                    }
                    ...
                    {
                        s5 = TextUtils.getValue(hashmap1, "name");
                        s7 = TextUtils.getValue(hashmap1, "value");
                    } else
        ...
        throws java.lang.Exception
    {
        ...
        {
            int j = com.dragonflow.Utils.TextUtils.toInt(TextUtils.getValue((jgl.HashMap)array3.at(0), "_URLRemoteSequenceMonitorSteps"));
        ...
        {
            atomicmonitor = AtomicMonitor.MonitorCreate(array3, request.getValue("id"), request.getPortalServer());
        ...
        if(nextStep > 1)
        {
            ...
            String s29 = null;
            if(TextUtils.getValue(hashmap, "_transFloatWindow").equalsIgnoreCase("CHECKED"))
            {
                s29 = Platform.getRoot() + "/htdocs/CurTransStep.html";
                s29 = s29.replace('\\', '/');
            }
        ...
        if(TextUtils.getValue(hashmap, "_transFloatWindow").equalsIgnoreCase("CHECKED") && j2 >= 0)
        {
            outputStream.println("<script language=\"JavaScript\"> URLfloat=window.open(\"http://" + TextUtils.getValue(hashmap, "_webserverAddress") + ":" + TextUtils.getValue(hashmap, "_httpActivePort") + com.dragonflow.SiteView.Platform.getURLPath("htdocs", request.getAccount()) + "/CurTransStep.html\",\"URLfloat\", \"scrollbars=yes, width=400, height=300, resizable=yes, status=yes, location=yes\");" + "</script>");
        }
        if(!request.getValue("AWRequest").equals("yes"))
        {
            printFooter(outputStream);
