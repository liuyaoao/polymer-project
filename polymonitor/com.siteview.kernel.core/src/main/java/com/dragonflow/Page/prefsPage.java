...
//    private static String topazShortName = TopazInfo
//            .getTopazNameShort();
//
//    private static String topazLongName = TopazInfo
//            .getTopazName();
    ...
    void printPrefsBar(String s) {
        ...
        String s1 = user.getProperty("_buttonBar");
        if (s1.length() > 0) {
            return;
        }
        ...
        outputStream.print(" | ");
        if (TextUtils.getValue(hashmap, "_haInstall")
                .length() > 0) {
...
//        }
        ...
        if (com.dragonflow.SiteView.Platform.isWindows()
            ...
            if (s.equals("NT Remotes")) {
                outputStream.println("<B>Remote Windows</B>");
            } else {
                outputStream
                        .println("<a href="
