...
public class machinePage extends com.dragonflow.Page.remoteBase
{
    ...
    String getTestMachineID()
    {
        ...
        String s1 = Machine.REMOTE_PREFIX + s;
    ...
    String getTestMachineName()
    {
        ...
        String s1 = Machine.getMachineName(s);
        return s1;
    }

    String getTestDescription()
    {
        String s = "<p>For this test, SiteView will attempt to run each of the common types of remote  monitors once on the remote machine.\n If these tests are successful, monitors created for the remote machine  will function properly.\n";
    ...
    {
        boolean flag = true;
        outputStream.println(field("Server Address", "<input type=text name=host size=50 value=\"" + TextUtils.getValue(hashmap, "_host") + "\">", "The server address of the remote server (for example, demo." + com.dragonflow.SiteView.Platform.exampleDomain + ")." + "  For HTTP connections, enter the URL of the CGI\n" + " (for example, http://demo." + com.dragonflow.SiteView.Platform.exampleDomain + "/cgi-bin/remote.sh). " + " To use the same login credentials for multiple servers, enter multiple server addresses " + " separated by commas. "));
        if(flag)
        {
    ...
    jgl.Array getListTableRow(jgl.HashMap hashmap, String s)
    {
        ...
        array.add(new String("<b>" + TextUtils.getValue(hashmap, "_name") + "</b>"));
    ...
    void doTest(com.dragonflow.SiteView.Machine machine)
    {
        ...
        {
            System.out.println("There was a problem updating the server status." + exception1.toString());
        }
    }
