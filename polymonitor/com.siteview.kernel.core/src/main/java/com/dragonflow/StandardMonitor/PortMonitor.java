...
//                    LogManager.log("RunMonitor", "Port Monitor sent to " + s + ":" + i + "(" + portType + "),  " + TextUtils.bytesToString(abyte0, abyte0.length));
                    ...
                    if (abyte1.length == 0) {
                        ...
                        l = as[0].contains(s3) ? URLMonitor.kURLok : URLMonitor.kURLContentMatchError;
                    }
        ...
        if (stillActive()) {
            synchronized (this) {
                ...
                if (l == URLMonitor.kURLok) {
                	str="status="+l+", roundTripTime(sec)="+TextUtils.floatToString((float) l2 / 1000F, 2);
                    String s5 = TextUtils.floatToString((float) l2 / 1000F, 2) + " sec";
                    setProperty(pStateString, s5);
                    setProperty(pRoundTripTime, l2);
                } else {
                	str="status("+ URLMonitor.lookupStatus(l)+")="+l+", roundTripTime(sec)="+TextUtils.floatToString((float) l2 / 1000F, 2);
                    setProperty(pRoundTripTime, "n/a");
                }
                setProperty(pStateString, str);
            }
        }
        return true;
    }
