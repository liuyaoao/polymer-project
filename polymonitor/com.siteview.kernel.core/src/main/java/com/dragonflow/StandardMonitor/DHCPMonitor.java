...
import com.dragonflow.Utils.PDH.DHCPClient;
...
public class DHCPMonitor extends AtomicMonitor {
    ...
    protected boolean update() {
        ...
        try {
			DHCPClient.getInstance().acquireAddress(getProperty(pRequestAddress), "", aobj, 1, 10, i, true);
    ...
    private boolean sendMessage(String s, DHCPMessage dhcpmessage, DHCPSocket dhcpsocket, int i, int j) {
        ...
        abyte1[0] = (byte) j;
        dhcpmessage.setOption(53, intToBytes(3));
        dhcpmessage.setOption(51, intToBytes(1));
        dhcpmessage.setDestinationHost("192.168.1.7");
        try {
