...
public class ScriptMonitor extends ServerMonitor {
    ...
    static StringProperty pScriptOutput_string1;
    static StringProperty pScriptOutput_string2;
    static NumericProperty pScriptOutput_int1;
    static NumericProperty pScriptOutput_int2;
...
//        }
        ...
        if (stillActive()) {
            synchronized (this) {
            	   if (as[0].trim().length() == 0) {
                       ...
                       int sk=0;
                       for (int k = 0; k < array.size() && (long) k < l2; k ++) {
                    	  String s17=(String) array.at(k);
                           s5 = s5 + s17;
                           s5 = s5 + "^";
                           try{
                          		if(ik==0){
                          			setProperty(pScriptOutput_int1, Integer.parseInt(s17));
                          			ik++;
                          		}else if(ik==1){
                          			setProperty(pScriptOutput_int2, Integer.parseInt(s17));
                          			ik++;
                          		}else if(sk==0){
                          			setProperty(pScriptOutput_string1, s17);
                          			sk++;
                          		}else if(sk==1){
                          			setProperty(pScriptOutput_string2, s17);
                          			sk++;
                          		}
                          	}catch (Exception e) {
                          		if(sk==0){
    ...
    public Array getLogProperties() {
        ...
        if (useStatusAndRoundtrip()) {
            ...
            array.add(pScriptOutput);
            array.add(pScriptOutput_int1);
            array.add(pScriptOutput_int2);
            array.add(pScriptOutput_string1);
            array.add(pScriptOutput_string2);
        }
    ...
    static {
        ...
        pScriptOutput_string1 = new StringProperty("stringvalue1");
        pScriptOutput_string1.setIsThreshold(true);
        arraylist.add(pScriptOutput_string1);
        
        pScriptOutput_string2 = new StringProperty("stringvalue2");
        pScriptOutput_string2.setIsThreshold(true);
        arraylist.add(pScriptOutput_string2);
        
        pScriptOutput_int1 = new NumericProperty("intvalue1", "0", "");
        pScriptOutput_int1.setLabel("scriptOutput int1");
        pScriptOutput_int1.setStateOptions(i ++);
        arraylist.add(pScriptOutput_int1);
        
        pScriptOutput_int2 = new NumericProperty("intvalue2", "0", "");
        pScriptOutput_int2.setLabel("scriptOutput int2");
        pScriptOutput_int2.setStateOptions(i ++);
        arraylist.add(pScriptOutput_int2);
        ...
        addClassElement(s1, Rule.stringToClassifier("intvalue1 >= 100\twarning"));
    ...
    public static Vector getScriptList(String s,String s1){
   	 Vector vector = new Vector();
        String s2 = s1;
        if (s2 == null || s2.length() == 0) {
            s2 = "scripts";
        ...
        if (Machine.isPortalMachineID(s)) {
            ...
            File file = new File(Platform.getUsedDirectoryPath(s2, "administrator"));
            String as[] = file.list();
            for (int k = 0; k < as.length; k ++) {
                String s6 = I18N.toNullEncoding(as[k]);
                if (s6.endsWith(".txt") || Platform.isUnix() && s6.startsWith(".")) {
