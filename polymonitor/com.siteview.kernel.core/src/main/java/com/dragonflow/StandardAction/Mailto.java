...
public class Mailto extends com.dragonflow.SiteView.Action {
    ...
    public String getActionString() {
        ...
        Enumeration enumeration = getMultipleValues(pTo);
    ...
    public java.util.Vector getScalarValues(com.dragonflow.Properties.ScalarProperty scalarproperty, com.dragonflow.HTTP.HTTPRequest httprequest, com.dragonflow.Page.CGI cgi) throws com.dragonflow.SiteViewException.SiteViewException {
        if (scalarproperty == pTemplate) {
            java.util.Vector vector = SiteViewGroup.getTemplateList("templates.mail", httprequest);
        ...
        if (scalarproperty == pTo) {
            ...
            for (; enumeration.hasMoreElements(); vector2.addElement(TextUtils.getValue(hashmap1, "_name"))) {
                hashmap1 = com.dragonflow.Utils.TextUtils.stringToHashMap((String) enumeration.nextElement());
                vector2.addElement(ID_STRING + TextUtils.getValue(hashmap1, "_id"));
            }
    ...
    public boolean execute() {
        boolean flag = false;
        String s = "unknown";
        String s2 = Platform.productName + " Alert, " + monitor.getProperty(com.dragonflow.SiteView.Monitor.pCategory) + ", " + monitor.getProperty(com.dragonflow.SiteView.Monitor.pName) + ", "
        ...
        for (int i = 0; i < as.length - 1; i ++) {
            ...
            if (s5.startsWith(ID_STRING)) {
                ...
                String s7 = TextUtils.getValue(hashmap, "_schedule");
                if (s7.length() > 0 && !com.dragonflow.Properties.ScheduleProperty.isEnabled(s7)) {
                    continue;
                }
                s5 = TextUtils.getValue(hashmap, "_email");
                s5.trim();
                if (s5.length() == 0) {
                    continue;
                }
                String s8 = TextUtils.getValue(hashmap, "_template");
