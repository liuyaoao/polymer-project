...
public class SiteViewXMLQuery
{
    ...
    public SiteViewXMLQuery(jgl.HashMap hashmap, com.dragonflow.HTTP.HTTPRequest httprequest)
    {
        ...
        if(TextUtils.getValue(hashmap, "remote").length() > 0)
        {
            localRequest = false;
            serverTitle = TextUtils.getValue(hashmap, "serverTitle");
            linkBase = "http://" + TextUtils.getValue(config, "_webserverAddress");
            String s = TextUtils.getValue(config, "_httpActivePort");
        ...
        if(serverFilter != null)
        {
            ...
            if(TextUtils.getValue(serverFilter, "all").length() > 0)
            {
    ...
    public jgl.Array getServers(jgl.HashMap hashmap)
    {
        ...
        {
            String s = Platform.getRoot() + "/groups/" + "multi.config";
            try
            {
        ...
        while (enumeration.hasMoreElements()) {
            ...
            String s1 = TextUtils.getValue(hashmap2, "server");
            if(s1.equals("_local") && TextUtils.getValue(hashmap2, "title").length() == 0)
            {
                hashmap2.put("title", com.dragonflow.SiteView.Server.LocalServerName());
            }
            if(TextUtils.getValue(hashmap2, "title").length() == 0)
            {
    ...
    public jgl.HashMap setupQueryMap(jgl.HashMap hashmap, String s)
    {
        ...
        String s1;
        for(Enumeration enumeration = hashmap.values(s); enumeration.hasMoreElements(); hashmapordered.put(s1, TRUE_STRING))
        {
    ...
    public void printXML()
    {
        ...
        for(Enumeration enumeration = servers.elements(); enumeration.hasMoreElements();)
        {
            ...
            if(!TextUtils.getValue(hashmap, "server").equals("_local"))
            {
    ...
    void printRemoteServer(jgl.HashMap hashmap)
    {
        ...
        String s6 = TextUtils.getValue(hashmap, "proxyusername");
        String s7 = TextUtils.getValue(hashmap, "proxypassword");
        String s8 = TextUtils.getValue(hashmap, "username");
        String s9 = TextUtils.getValue(hashmap, "password");
        String s10 = TextUtils.getValue(hashmap, "timeout");
        int i = 60000;
        if(s10.length() != 0)
        {
        ...
        if(s8.length() == 0)
        {
            s8 = TextUtils.getValue(config, "_adminUsername");
            s9 = TextUtils.getValue(config, "_adminPassword");
        }
        long l = -1L;
        if(TextUtils.getValue(config, "_overviewMaxData").length() > 0)
        {
            l = com.dragonflow.Utils.TextUtils.toLong(TextUtils.getValue(config, "_overviewMaxData"));
        }
        if(l < 1L)
        {
            l = 0x7a120L;
        ...
        if(expressionFilter != null)
        {
            for(Enumeration enumeration = expressionStringFilter.elements(); enumeration.hasMoreElements(); array.add(enumeration.nextElement())) { }
        }
        array.add("remote=true");
        array.add("serverTitle=" + TextUtils.getValue(hashmap, "title"));
    ...
    com.dragonflow.SiteView.Action createAction(com.dragonflow.SiteView.Rule rule)
    {
        ...
        com.dragonflow.SiteView.Action action = Action.createAction(s);
