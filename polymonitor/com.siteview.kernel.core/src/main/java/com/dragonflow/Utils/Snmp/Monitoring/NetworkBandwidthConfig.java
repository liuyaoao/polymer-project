...
public class NetworkBandwidthConfig {
    ...
    public static java.util.Vector getIDsandDisplayNames() {
        ...
        String s;
        for (Enumeration enumeration = deviceIDToDisplayName.keys(); enumeration.hasMoreElements(); vector.add(deviceIDToDisplayName.get(s))) {
    ...
    static {
        configFilePath = Platform.getRoot() + java.io.File.separator + "templates.applications" + java.io.File.separator + "netbandwidth.xml";
        configDocument = null;
        ...
        int k = com.dragonflow.Utils.TextUtils.toInt(TextUtils.getValue(hashmap, "_NetworkBandwidthMaxCustomCounters"));
        if (k > 0) {
            maxProperties = k;
        }
    }
