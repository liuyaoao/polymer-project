...
public class TempFileManager implements java.lang.Runnable {
    ...
    private static boolean timeHasExpired(java.io.File file, long l) {
        ...
        String as[] = s.split("--");
        if (as.length != 3) {
            return file.exists() && System.currentTimeMillis() - file.lastModified() > l * 60L * 60L * 1000L;
        }
    ...
    public void run() {
        ...
        int j = com.dragonflow.Utils.TextUtils.toInt(TextUtils.getValue(hashmap, "_tempDirMaxSize"));
        int k = com.dragonflow.Utils.TextUtils.toInt(TextUtils.getValue(hashmap, "_defaultTempFileAge"));
        if (k <= 0) {
            k = 24;
        }
        j *= 1024;
