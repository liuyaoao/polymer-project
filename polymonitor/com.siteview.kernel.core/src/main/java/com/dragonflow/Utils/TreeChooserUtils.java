...
public class TreeChooserUtils {
    ...
    public TreeChooserUtils(com.dragonflow.HTTP.HTTPRequest httprequest, java.io.PrintWriter printwriter, jgl.Array array) {
        ...
        groupHashMap = null;
        conditions = null;
        siteview = SiteViewGroup.currentSiteView();
        request = httprequest;
    ...
    private String prepareTree(String s, String s1, jgl.Array array, jgl.HashMap hashmap) {
        ...
        if (com.dragonflow.SiteView.Platform.isStandardAccount(request.getAccount())) {
            s2 = Platform.getDirectoryPath("groups", request.getAccount());
        } else {
            s2 = Platform.getRoot() + java.io.File.separator + "accounts" + java.io.File.separator + request.getAccount();
        }
    ...
    private void updateCurrentStateFrames(jgl.Array array, jgl.HashMap hashmap) {
        ...
        for (int i = 0; i < array.size(); i ++) {
            ...
            if (TextUtils.getValue(hashmap2, "_user").equals(s)) {
                hashmap1 = hashmap2;
            }
        }
        ...
        while (enumeration.hasMoreElements()) {
            ...
            if (s1.startsWith(com.dragonflow.Page.managePage.OPEN_VARIABLE)) {
                ...
                if (!TextUtils.getValue(hashmap1, s2).equals("open")) {
            ...
            if (s1.startsWith(com.dragonflow.Page.managePage.CLOSE_VARIABLE)) {
                ...
                if (!TextUtils.getValue(hashmap1, s3).equals("close")) {
                    hashmap1.remove(s3);
                }
            }
        } 
                ...
                if (s2.equals("_master")) {
                    ...
                    if (s3 != null) {
                        ...
                        for (int l = 0; l < array2.size(); l ++) {
                            jgl.Array array3 = Platform.split(' ', (String) array2.at(l));
        ...
        while (enumeration.hasMoreElements()) {
            ...
            if (flag) {
                ...
                try {
                    ...
                    jgl.Array array2 = Platform.split(' ', s2);
                    if (array2.size() == 1) {
                        array1.add(s2);
                    } else {
                    ...
                    while (enumeration1.hasMoreElements()) {
                        ...
                        jgl.Array array4 = Platform.split(' ', s4);
                        if (array4.size() == 1) {
        ...
        if (flag) {
            ...
            if (s5.equals("_config")) {
                ...
                if (s6 == null || s6.length() == 0) {
                    ...
                    jgl.Array array1 = Platform.split(',', s6);
                    for (int k = 0; k < array1.size(); k ++) {
                        if (((String) array1.at(k)).indexOf(s3) >= 0) {
                            jgl.Array array2 = Platform.split(' ', (String) array1.at(k));
                            if (array2.size() > 1) {
    ...
    private void getReportMonitors(String s, com.dragonflow.SiteView.MonitorGroup monitorgroup, jgl.Array array) {
        if (s != null) {
            jgl.Array array1 = Platform.split(' ', s);
            if (array1.size() == 1) {
    ...
    private String printGroup(String s, String s1, jgl.HashMap hashmap, jgl.HashMap hashmap1, int i, String s2) {
        ...
        String s4 = Platform.getURLPath("htdocs", request.getAccount()) + "/Detail";
        outputStream.print("<TR><TD>");
        outputStream.print(s3);
        printOpenClose(flag, s);
        if (s2.equals("Group")) {
            outputStream.print("<input type=checkbox name=groupID value=\"" + s + "\" " + TextUtils.getValue(hashmap1, s) + ">");
    ...
    private void printMonitor(com.dragonflow.SiteView.Monitor monitor, jgl.HashMap hashmap, String s, String s1, String s2) {
        ...
        if (s1.equals("Group")) {
            outputStream.print("<input type=checkbox name=monitorID value=\"" + s3 + "\" " + TextUtils.getValue(hashmap, s3) + ">");
        }
