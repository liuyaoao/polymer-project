...
REM * than copies the Web Server binary files, that were already copied to a temp dir under SiteView
REM * and run the sarmanalyzer in order to generate the User Session Trace xml files.
REM * In order to be invoked by a SiteView script monitor, this file should reside under the
REM * 'templates.applications\scripts.siebel' directory of SiteView.
REM *
REM * Usage:
REM *   %1 - SiteView temp dir
...
REM *
REM * Usage example:
REM *   C:\SiteView\templates.applications\scripts.siebel\collect_sarm_web_files.bat \templates.eventlog
...
REM the batch file name
    set batch_name=%0
REM keep the SiteView temp dir, to which the files should be copied.
    set ss_temp_dir=..\%1
